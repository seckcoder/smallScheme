...
(library (Framework GenGrammars l28-pre-assign-frame)
  (export verify-grammar:l28-pre-assign-frame)
  ...
  (define verify-grammar:l28-pre-assign-frame
    (lambda (x)
      (define Tail
        (lambda (x)
          (match x
            ...
            [(locals
               ...
               (new-frames
                 ...
                 (locate
                   ...
                   (frame-conflict
                     ((,(UVar -> x5) ,(Var -> x6) ...) ...)
                     (call-live (,(UFVar -> x7) ...) ,(Tail -> x8))))))
             (any x8 x7 x6 x5 x4 x3 x2 x1)]
            [,e (invalid-expr 'Body e)])))
      (let ([res (Prog x)])
        (if res
            (errorf 'verify-grammar:l28-pre-assign-frame "~a" res)
            x)))))
