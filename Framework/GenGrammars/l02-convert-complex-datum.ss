...
(library (Framework GenGrammars l02-convert-complex-datum)
  (export verify-grammar:l02-convert-complex-datum)
  ...
  (define verify-grammar:l02-convert-complex-datum
    (lambda (x)
      (define Prog
        (lambda (x)
          (match x
      ...
      (let ([res (Prog x)])
        (if res
            (errorf 'verify-grammar:l02-convert-complex-datum "~a" res)
            x)))))
