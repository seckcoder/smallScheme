...
(library (Framework GenGrammars l04-purify-letrec)
  (export verify-grammar:l04-purify-letrec)
  ...
  (define verify-grammar:l04-purify-letrec
    (lambda (x)
      (define Prog
        (lambda (x)
          (match x
      ...
      (define Expr
        (lambda (x)
          (match x
            ...
            [',(Immediate -> x1) (any x1)]
            [(letrec ([,(UVar -> x1) ,(Lamb -> x2)] ...) ,(Body -> x3))
             (any x3 x2 x1)]
      ...
      (let ([res (Prog x)])
        (if res
            (errorf 'verify-grammar:l04-purify-letrec "~a" res)
            x)))))
