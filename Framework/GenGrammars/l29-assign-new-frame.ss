...
(library (Framework GenGrammars l29-assign-new-frame)
  (export verify-grammar:l29-assign-new-frame)
  ...
  (define verify-grammar:l29-assign-new-frame
    (lambda (x)
      (define Tail
        (lambda (x)
          (match x
      ...
      (let ([res (Prog x)])
        (if res
            (errorf 'verify-grammar:l29-assign-new-frame "~a" res)
            x)))))
